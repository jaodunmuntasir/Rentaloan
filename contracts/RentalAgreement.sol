...
contract RentalAgreement is IRentalAgreement, ReentrancyGuard {
     ...
     */
    function receiveRentFromLoan() external payable override contractActive nonReentrant {
        uint256 paymentAmount = _baseRent.add(dueAmount);
        require(msg.value == paymentAmount, "RentalAgreement: Incorrect rent amount");
        ...
        // Transfer to landlord
        (bool success, ) = landlord.call{value: paymentAmount}("");
        require(success, "RentalAgreement: Transfer to landlord failed");
    ...
    function getContractDetails() external view returns (
        address, address, uint256, uint256, uint256, uint256, ContractStatus, uint256, uint256, uint256, uint256, uint256
    ) {
        return (
            landlord,
            renter,
