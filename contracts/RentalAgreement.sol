...
contract RentalAgreement is IRentalAgreement, ReentrancyGuard {
    ...
    enum PaymentMethod { WALLET, LOAN }
    
    event ContractCreated(address landlord, address renter, uint256 duration, uint256 deposit, uint256 rent);
    ...
    event ContractClosed(string reason, uint256 renterReturn, uint256 landlordPayment);
    event SecurityDepositUpdated(uint256 newAmount);
    event DurationExtended(uint256 newDuration);
    
    modifier onlyLandlord() {
        require(msg.sender == landlord, "RentalAgreement: Only landlord can call");
        _;
    }
    ...
    modifier contractActive() {
        require(_status == ContractStatus.ACTIVE, "RentalAgreement: Contract is not active");
        _;
    }
    ...
    constructor(
        ...
        require(_duration > 0, "RentalAgreement: Duration must be greater than 0");
        require(_securityDeposit >= _baseRent, "RentalAgreement: Security deposit must be >= base rent");
        require(_gracePeriod > 0 && _gracePeriod <= _duration.div(2), "RentalAgreement: Invalid grace period");
        require(_loanAgreementFactory != address(0), "RentalAgreement: Invalid loan factory address");
    ...
    function paySecurityDeposit() external payable onlyRenter nonReentrant {
        require(_status == ContractStatus.INITIALIZED, "RentalAgreement: Contract already initialized");
        require(msg.value == securityDeposit, "RentalAgreement: Incorrect security deposit amount");
     ...
     */
    function payRent(uint256 month, PaymentMethod method) external payable contractActive nonReentrant {
        require(month > lastPaidMonth && month <= rentalDuration, "RentalAgreement: Invalid month");
        require(!rentPaidForMonth[month], "RentalAgreement: Rent already paid for this month");
        require(!rentSkippedForMonth[month], "RentalAgreement: Rent already skipped for this month");
        
        uint256 unpaidMonths = month - lastPaidMonth;
        require(unpaidMonths <= gracePeriod, "RentalAgreement: Too many unpaid months");
        
        uint256 paymentAmount = _baseRent.add(dueAmount);
        
        if (method == PaymentMethod.WALLET) {
            require(msg.value == paymentAmount, "RentalAgreement: Incorrect rent amount");
            ...
            require(msg.value == 0, "RentalAgreement: Loan payment should not send ETH");
    ...
    function skipRent(uint256 month) external contractActive onlyRenter {
        ...
        require(!rentPaidForMonth[month], "RentalAgreement: Rent already paid for this month");
        require(!rentSkippedForMonth[month], "RentalAgreement: Rent already skipped for this month");
        require(month < rentalDuration, "RentalAgreement: Cannot skip the final month");
        require(skippedMonths < gracePeriod, "RentalAgreement: Exceeded maximum allowed skipped months");
     ...
     * @param month The month for which rent is being paid
     */
    function receiveRentFromLoan(uint256 month) external payable override contractActive nonReentrant {
        require(msg.value == _baseRent, "RentalAgreement: Incorrect rent amount");
        require(ILoanAgreementFactory(loanAgreementFactory).isValidLoanContract(msg.sender), "RentalAgreement: Invalid loan contract");
        ...
        emit RentPaid(month, _baseRent, dueAmount);
    }
     ...
     */
    function withdrawCollateral(uint256 amount) external override contractActive nonReentrant {
        require(ILoanAgreementFactory(loanAgreementFactory).isValidLoanContract(msg.sender), "RentalAgreement: Invalid loan contract");
        require(amount <= getAvailableCollateral(), "RentalAgreement: Insufficient collateral");
     ...
     */
    function returnCollateral(uint256 amount) external payable override contractActive nonReentrant {
        require(ILoanAgreementFactory(loanAgreementFactory).isValidLoanContract(msg.sender), "RentalAgreement: Invalid loan contract");
     ...
     */
    function closeContractEarly(string memory reason) external contractActive nonReentrant {
        require(msg.sender == landlord || msg.sender == renter, "RentalAgreement: Only landlord or renter can close early");
     ...
     */
    function extendContractDuration(uint256 additionalMonths) external onlyLandlord contractActive {
        require(additionalMonths > 0, "RentalAgreement: Additional months must be greater than 0");
        require(lastPaidMonth == rentalDuration - 1, "RentalAgreement: Can only extend in the last month");
     ...
     */
    function getCurrentSecurityDeposit() external view override returns (uint256) {
     ...
     */
    function getContractStatus() external view override returns (ContractStatus) {
        return _status;
    }
    ...
    function _closeContract(string memory reason) internal {
        ...
        if (lastPaidMonth < rentalDuration) {
            dueRent = dueAmount.add(_baseRent.mul(rentalDuration.sub(lastPaidMonth)));
        } else {
            dueRent = dueAmount;
        }
    ...
    function getContractDetails() external view returns (
        address, address, uint256, uint256, uint256, uint256, ContractStatus, uint256, uint256, uint256
    ) {
        return (
            landlord,
            renter,
